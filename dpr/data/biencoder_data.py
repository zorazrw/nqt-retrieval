...
def get_nq_table_files(source_name: str) -> List[str]:
    if os.path.exists(source_name) or glob.glob(source_name):
        return glob.glob(source_name)
    else:
        logger.error(f"cannot locate data files in name {source_name}")


TableCell = collections.namedtuple("TableCell", ["text", "row_idx", "col_idx"])
BiEncoderTable = collections.namedtuple("BiEncoderTable", ["cells", "title"])
# "cells": [{"text": "cell string", "row_idx": 0-index (w/ header), "col_idx": 0-index}, {}, ..., {}]
...
def normalize(text: str) -> str: 
    ...
    Alter "NFD" to "NFKD", to normalize '\xa0' to ' ' (space). 
    """ 
    return unicodedata.normalize("NFKD", text) 


import spacy 
nlp = spacy.load("en_core_web_sm", disable=["parser", "tagger", "ner", "entity_ruler", "lemmatizer"]) 
# add disable 'lemmatizer', following: 'https://stackoverflow.com/questions/66451577/warning-w108-the-rule-based-lemmatizer-did-not-find-pos-annotation-for-the-to'
...
def string_truncate(text: str, max_tokens: int = None) -> List[str]: 
    """Normalize, tokenize, and skip randomly. 
    args: 
        text <str>: input cell string text. 
        max_tokens <int>: maximum number of (nlp-)tokens to include. 
            default `8` for content rows, `12` for header cells. 
    rets: 
        words <List[str]>: list of words after length truncation. 
    """ 
    words = tokenize(normalize(text)) 
    words = [normalize_passage(w) for w in words]
    words = [w.strip() for w in words]
    words = [w for w in words if w]
    if max_tokens: words = words[: max_tokens] 
    return words
...
def get_row_list(
    cells: List, 
    row_index: int, 
    max_words_per_cell: int = None, 
    max_cell_num: int = None, 
    cell_delimiter: str = '', 
    row_delimiter: str = '', 
    ...
    if not max_cell_num: max_cell_num = len(cells)
    else: max_cell_num = min(max_cell_num, len(cells))

    for j, cell in enumerate(cells[: max_cell_num]): 
        # print(f"text before: {cell['text']}")
        words = string_truncate(cell['text'])  
        if not words: continue 
        if max_words_per_cell: words = words[: max_words_per_cell]
        num_words += len(words) 
        # print(f"get words: {words}")

        if (j == max_cell_num-1) and row_delimiter: 
            words.append(row_delimiter) 
            num_words += 1 
        elif cell_delimiter: 
            words.append(cell_delimiter)
            num_words += 1 
...
def select_row_indices(num_rows: int, option: str): 
    row_indices = [i for i in range(num_rows)]
    if option == 'random': 
...
def get_cell_list(
    table: Dict, 
    row_selection: str = 'none',  # ['none', 'random', 'ngram']
    max_cell_num: int = None, 
    max_words: int = None, 
    max_words_per_header: int = None, 
    max_words_per_cell: int = None, 
    max_cell_num_per_row: int = None, 
    ...
    header_dict = get_row_list(
        table['columns'], 
        row_index=0, 
        max_words_per_cell=max_words_per_header, 
        max_cell_num=max_cell_num_per_row, 
        cell_delimiter=header_delimiter, 
        row_delimiter=row_delimiter, 
        return_dict=return_dict, 
    )
    cell_list.extend(header_dict['cells'])
    num_words += header_dict['num_words']
    
    num_rows = min(len(table['rows']), 32)
    row_indices = select_row_indices(num_rows, row_selection)
    for i, rid in enumerate(row_indices): 
        row = table['rows'][rid]['cells']
        row_dict = get_row_list(
            row, 
            row_index=i+1, 
        ...
        cell_list.extend(row_dict['cells'])
        num_words += row_dict['num_words'] 
        
        if (max_words) and (num_words >= max_words): 
            break 
    
    if max_cell_num: cell_list = cell_list[: max_cell_num]
    return cell_list

def get_processed_table(table: Dict, **kwargs): 
    cells = get_cell_list(table, **kwargs)
    title = ' '.join(string_truncate(table['documentTitle'])) 
    return BiEncoderTable(cells, title)


def get_processed_table_dict(table: Dict, **kwargs): 
    cells = get_cell_list(table, **kwargs)
    title = ' '.join(string_truncate(table['documentTitle'])) 
...
class BiEncoderTableSample(object):
    query: str
    positive_passages: List[BiEncoderTable]
    negative_passages: List[BiEncoderTable]
    hard_negative_passages: List[BiEncoderTable]


class TableJsonQADataset(Dataset):
    def __init__(
        self,
        file: str,
        selector: DictConfig = None,
    ...
    def _load_all_data(self):
        ...
        data = read_data_from_json_files(self.data_files)
        # filter those without positive ctx
        self.data = [r for r in data if any([ctx['has_answer'] for ctx in r['ctxs']])]
        logger.info("Total cleaned data size: %d", len(self.data))
        self.process_dataset() 
        logger.info("finished pre-processing all dataset samples")
    
    def process_dataset(self): 
        processed_data = [self.process_sample(sample) for sample in self.data] 
        self.data = processed_data 
        
    def process_sample(self, json_sample): 
        r = BiEncoderTableSample()
        r.query = self._process_query(json_sample["question"])

        positive_ctxs, negative_ctxs, hard_negative_ctxs = [], [], [] 
        for idx, ctx in enumerate(json_sample['ctxs']): 
        ...
        for ctx in positive_ctxs + negative_ctxs + hard_negative_ctxs:
            if "title" not in ctx:
                ctx["title"] = ctx['table']['documentTitle'] 

        def create_table(ctx: dict):
            return get_processed_table(
                table=ctx['table'], 
                row_selection='none', 
                max_cell_num=120, 
                max_words=120, 
                ...
                max_cell_num_per_row=None, 
                header_delimiter='|', 
                cell_delimiter='|', 
                row_delimiter='.', 
            )

        r.positive_passages = [create_table(ctx) for ctx in positive_ctxs]
        r.negative_passages = [create_table(ctx) for ctx in negative_ctxs]
        r.hard_negative_passages = [create_table(ctx) for ctx in hard_negative_ctxs]
        return r 

    def __getitem__(self, index) -> BiEncoderTableSample:
        return self.data[index]


class NqtJsonQADataset(Dataset):
    def __init__(
        self,
        file: str,
        selector: DictConfig = None,
    ...
    def _load_all_data(self):
        ...
        # self.data = [r for r in data if any([ctx['has_answer'] for ctx in r['ctxs']])]
        self.data = [r for r in data if len(r["positive_ctxs"]) > 0]
        self.data = [r for r in data if len(r["hard_negative_ctxs"]) > 0]
        self.data = [r for r in data if len(r["negative_ctxs"]) > 0]
        logger.info("Total cleaned data size: %d", len(self.data))
        self.process_dataset() 
        logger.info("finished pre-processing all dataset samples")
    ...
    def process_sample(self, json_sample): 
        r = BiEncoderTableSample()
        r.query = self._process_query(json_sample["question"])
        r.positive_passages = json_sample['positive_ctxs']
        r.negative_passages = json_sample['negative_ctxs']
        r.hard_negative_passages = json_sample['hard_negative_ctxs']
        return r 
