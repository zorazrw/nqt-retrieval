...
def init_hf_bert_mix_biencoder(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_bert_mix_biencoder_components

    return get_bert_mix_biencoder_components(args, **kwargs)


def init_hf_bert_bias_biencoder(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_bert_bias_biencoder_components

    return get_bert_bias_biencoder_components(args, **kwargs)
...
BIENCODER_INITIALIZERS = {
    ...
    "hf_bert_mix": init_hf_bert_mix_biencoder, 
    "hf_bert_bias": init_hf_bert_bias_biencoder, 
}
