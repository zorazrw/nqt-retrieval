...
from dpr.data.table_data import BiEncoderTable, prepare_table_ctx_inputs
...
BiEncoderBatchTable = collections.namedtuple(
    "BiEncoderInputTable",
    [
        "question_ids",
        "question_segments",
...
BiEncoderBatchTableAuxEmb = collections.namedtuple(
    "BiEncoderInputTable",
    [
        "question_ids",
        "question_segments",
...
TableBiEncoderBatch = collections.namedtuple(
    "TableBiEncoderBatch",
    [
        "question_ids", "question_segments",
        "context_ids", "ctx_segments", "ctx_attn_masks", 
        "ctx_row_ids", "ctx_column_ids", 
...
class BiEncoder(nn.Module):
    ...
    def forward(
        ...
        _ctx_seq, ctx_pooled_out, _ctx_hidden = self.get_representation(
            ctx_encoder, context_ids, ctx_segments, ctx_attn_mask, ctx_row_ids, ctx_column_ids, 
    ...
    def create_table_input(
        ...
        question_tensors, ctx_tensors = [], [] 
        ctx_attn_masks, ctx_row_ids, ctx_column_ids = [], [], []
        ...
        for sample in samples: 
            ...
            for ctx in all_ctxs: 
                ctx_inputs = prepare_table_ctx_inputs(
                    ctx['table'], 
                    tokenizer=tensorizer.tokenizer, 
                    structure_option=structure_option, 
                    insert_title=insert_title, 
                )
                ctx_tensors.append(ctx_inputs['token_ids'])
                ctx_attn_masks.append(ctx_inputs['attn_mask'])
                ctx_row_ids.append(ctx_inputs['row_ids'])
                ctx_column_ids.append(ctx_inputs['column_ids'])
        ...
        ctx_tensors_batch = torch.cat([ctx.view(1, -1) for ctx in ctx_tensors], dim=0)
        # ctx_row_ids_batch = torch.cat([ctx.view(1, -1) for ctx in ctx_row_ids], dim=0)
        ctx_row_ids_batch = torch.cat([ctx.unsqueeze(0) for ctx in ctx_row_ids], dim=0)
        ctx_column_ids_batch = torch.cat([ctx.view(1, -1) for ctx in ctx_column_ids], dim=0)
        ctx_attn_masks_batch = torch.cat([mask.unsqueeze(0) for mask in ctx_attn_masks], dim=0)
        
        question_tensors_batch = torch.cat([q.view(1, -1) for q in question_tensors], dim=0)
        
        positive_ctx_indices = torch.LongTensor(positive_ctx_indices)
        hard_neg_ctx_indices = torch.LongTensor(hard_neg_ctx_indices)

        ctx_segments_batch = torch.zeros_like(ctx_tensors_batch)
        question_segments_batch = torch.zeros_like(question_tensors_batch)
        
        return TableBiEncoderBatch(
            question_ids=question_tensors_batch,
            question_segments=question_segments_batch,
            context_ids=ctx_tensors_batch,
            ctx_segments=ctx_segments_batch,
            ctx_attn_masks=ctx_attn_masks_batch, 
            ctx_row_ids=ctx_row_ids_batch, 
            ctx_column_ids=ctx_column_ids_batch,
...
class BiEncoderNllLoss(object):
    def calc(
        ...
        # print(f"softmax_scores ({softmax_scores.shape}): {softmax_scores}")
        # print(f"positive_idx_per_question ({positive_idx_per_question.shape}): {positive_idx_per_question}")
