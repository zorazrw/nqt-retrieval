...
def get_optimizer(
    ...
    if emb_only: 
        optimizer_grouped_parameters = get_hf_model_param_grouping_selected(model, weight_decay)
    else: 
...
def get_hf_model_param_grouping_selected(
    model: nn.Module,
    weight_decay: float = 0.0,
    selected: List[str] = ["row_embeddings", "column_embeddings"]
):
    no_decay = ["bias", "LayerNorm.weight"]
    
    named_params = [(n,p) for n,p in model.named_parameters() if any([s in n for s in selected])]
...
class HFEmbedEncoder(BertModel):
    def __init__(self, config, project_dim: int = 0):
        BertModel.__init__(self, config)
        assert config.hidden_size > 0, "Encoder hidden_size can't be zero"
    ...
    def init_encoder(
        ...
        if pretrained:
            ...
            return HFEmbedEncoder(cfg, project_dim=projection_dim)
...
def get_bert_mix_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
    question_encoder = HFBertEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
    ...
    ctx_encoder = HFEmbedEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
    ...
    if inference_only: 
        ...
        optimizer = get_optimizer(
            ...
            adam_eps=cfg.train.adam_eps,
            weight_decay=cfg.train.weight_decay,
            emb_only=cfg.encoder.auxiliary_embeddings_only, 
        )
...
class HFBiasEncoder(BertModel):
    ...
    def get_biased_attenion_mask(self, biased_mask_id: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor: 
        ...
        attention_mask = (1.0 - attention_mask) * -10000.0    # global mask 
...
def get_bert_bias_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
    question_encoder = HFBertEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
    ...
    ctx_encoder = HFBiasEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
