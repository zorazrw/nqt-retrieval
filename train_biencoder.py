"""Train the Bi-Encoder using table data. """
...
from dpr.models.biencoder import TableBiEncoderBatch, BiEncoderNllLoss
...
def _do_biencoder_fwd_pass_mask(
    model: nn.Module,
    input: TableBiEncoderBatch,
    ...
    input = TableBiEncoderBatch(**move_to_device(input._asdict(), cfg.device))

    q_attn_mask = tensorizer.get_attn_mask(input.question_ids).to(cfg.device)
    if cfg.structure_option == "auxemb": 
        ctx_attn_mask = tensorizer.get_attn_mask(input.context_ids) 
        ctx_row_ids = input.ctx_row_ids
        ctx_column_ids = input.ctx_column_ids 
    elif cfg.structure_option == "biased": 
        ctx_attn_mask = tensorizer.get_attn_mask(input.context_ids) 
        ctx_row_ids = input.ctx_row_ids
        ctx_column_ids = None 
    elif cfg.structure_option == 'rowcol': 
        ctx_attn_mask = input.ctx_attn_masks 
        ctx_row_ids = None 
        ctx_column_ids = None 
    else:    # 'global'
        ctx_attn_mask = tensorizer.get_attn_mask(input.context_ids) 
    ...
    if model.training:
        model_out = model(
            ...
            ctx_row_ids=ctx_row_ids, 
            ctx_column_ids=ctx_column_ids, 
...
class BiEncoderTrainer(object):
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            biencoder_input = biencoder.create_table_input(
                samples=samples_batch, 
                tensorizer=self.tensorizer, 
                structure_option=cfg.structure_option, 
                insert_title=True, 
            ...
            loss, correct_cnt = _do_biencoder_fwd_pass_mask(
                self.biencoder,
                biencoder_input,
                self.tensorizer,
                cfg,
    ...
    def validate_average_rank(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            if cfg.structure_option == 'auxemb': 
                ctx_attn_masks = biencoder_input.ctx_attn_masks
                ctx_row_ids = biencoder_input.ctx_row_ids
                ctx_column_ids = biencoder_input.ctx_column_ids 
            else: 
                ctx_attn_masks = biencoder_input.ctx_attn_masks
                ctx_row_ids = None
                ctx_column_ids = None
            ...
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):
                ...
                q_attn_mask = self.tensorizer.get_attn_mask(q_ids).to(q_ids.device)
                ctx_attn_mask = ctx_attn_masks[batch_start : batch_start + sub_batch_size]
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            biencoder_batch = biencoder.create_table_input(
                samples=samples_batch, 
                tensorizer=self.tensorizer, 
                structure_option=cfg.structure_option, 
                insert_title=True, 
    ...
    def _load_saved_state(self, saved_state: CheckpointState):
        ...
        model_to_load.load_state(saved_state, strict=False)   #@zora: True
...
@hydra.main(config_path="conf", config_name="biencoder_train")
